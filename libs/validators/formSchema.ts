...
export const addFormSchema = z.object({
    ...
    skills: z.string().min(1, "Au moins une compétence est obligatoire").refine(areSkillsValid, "Au moins une compétences est invalide"),
    type: z.string().refine(isTypeValid, "Le type de contrat n'est pas valide"),
    start_date: z.coerce.date("La date est obligatoire").min(new Date(), "La date doit être a minima la date du jour"),
    salary: z.coerce.number("Le salaire doit être un nombre").min(0, "Un salaire positif est obligatoire"),
